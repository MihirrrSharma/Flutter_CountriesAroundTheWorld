...
import 'package:countries_of_the_world/helper/getExchangeRates.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _loadCountries() async {
    ...
    exchangeRate = await getExchangeRatesUsingUsersLocation();
  }
  ...
  Future getExchangeRatesUsingUsersLocation() async {
    rates = await getExchangeRates();
    debugPrint("rates inside getExchangeRatesUsingUsersLocation: $rates");
    ...
    baseCurrency = getCurrencyCode(countryCode!);
    debugPrint("baseCurrency: $baseCurrency");

    double xchangeRate = rates['rates'][baseCurrency];
    debugPrint("xchangeRate: $xchangeRate");
    return xchangeRate;
  }
  ...
  Future<void> _showCountryDetails(Map<String, dynamic> country) async {
    ...
    try {
      var currentCurrency = getCurrencyCode(weather["sys"]["country"]);
      debugPrint("currentCurrency: $currentCurrency");

      double xchangeRate = rates['rates'][currentCurrency];
      var xchangeRateInfo = '1 USD = $xchangeRate $currentCurrency';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: exchangeRate == null
          ? null
          : Container(
              ...
              child: Center(
                child: Text(
                  "Your Local Exchange Rate: 1 USD = $exchangeRate $baseCurrency",
                  style: const TextStyle(
                    fontWeight: FontWeight.bold,
                    fontSize: 12,
                    color: Colors.white,
